...
class DeviceNotFoundAlert(NotebookAlert):
    def __init__(self, device: str):
        ...
        if len(supported_devices) == 1:
            self.message += (
                f"The following device is available: {core.available_devices[0]}"
            )
        else:
            self.message += (
                "The following devices are available: "
                f"{', '.join(core.available_devices)}"
            )
            ...
            def callback(matcher: Matcher) -> bool:
                ...
                for y in packed_layername_tensor_dict_list:
                    ...
                    if root_name.find(y["name"]) != -1:
                        ...
                        new_tenser = ops.constant(
                            max_fp16, ov.Type.f32, name="Constant_4431"
                        )
